...
<div class="modal fade" id="addScheduleModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            </div>            <form action="{{ route('admin.schedules.store') }}" method="POST" id="addScheduleForm">
...
if (typeof bootstrap === 'undefined') {
    ...
    fetch(`/admin/schedules/${scheduleId}`)
        ...
        .then(data => {        // Format days of week and specific dates
        const days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
        let operationInfo = '';
        
        if (data.use_specific_dates) {
            ...
            // Format days of week for recurring schedules
            let daysOfWeek = [];
            ...
            operationInfo = daysOfWeek.length > 0 ? daysOfWeek.join(', ') + ' <small class="text-muted">(Berulang)</small>' : 'Tidak ada informasi';
        }
...
function updateHiddenInputs() {
    ...
    const form = document.querySelector('form[action="{{ route('admin.schedules.store') }}"]');
    selectedDates.forEach(date => {
